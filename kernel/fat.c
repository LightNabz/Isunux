 ...
 * would find. */
typedef int (*fat_dirent_visitor_t)(const char *display_name, const uint8_t *raw_entry, int real_index, void *ctx);

/* Scans one already-read 512-byte directory sector. Sets *stop to 1
 * if the true end-of-directory marker (name[0]==0x00) was hit, or the
 * visitor itself asked to stop -- either way the caller must not read
 * any further sectors after that. */
static void fat_scan_sector_dirents(const uint8_t *sector, fat_dirent_visitor_t visitor,
    ...
    for (int off = 0; off < 512; off += 32) {
        ...
        if (visitor(display, entry, *real_index, ctx)) { *stop = 1; return; }
        (*real_index)++;
    }
}
...
static void fat_iterate_dir(vnode_t *dir_vnode, fat_dirent_visitor_t visitor, void *ctx) {
    ...
    if (meta->first_cluster == 0) {
        for (uint32_t s = 0; s < root_dir_sectors && !stop; s++) {
            if (ata_read_sectors(first_root_dir_sector + s, 1, sector) != 0) return;
            fat_scan_sector_dirents(sector, visitor, ctx, &real_index, &stop);
        }
        return;
    }
    ...
    while (cluster >= 2 && cluster < FAT16_EOC_MIN && !stop) {
        ...
        for (uint8_t s = 0; s < sectors_per_cluster && !stop; s++) {
            if (ata_read_sectors(lba + s, 1, sector) != 0) return;
            fat_scan_sector_dirents(sector, visitor, ctx, &real_index, &stop);
        }
...
static int fat_lookup_visitor(const char *display_name, const uint8_t *raw_entry, int real_index, void *ctx_) {
    (void)real_index;
    ...
    /* uid/gid stay 0 (root) -- this is read-only foreign media populated
...
static int fat_readdir_visitor(const char *display_name, const uint8_t *raw_entry, int real_index, void *ctx_) {
    (void)raw_entry;
...
static vnode_ops_t fat_file_ops = {
    .read = fat_file_read,
    .write = NULL, /* read-only filesystem -- see fat.h's top comment */
    .lookup = NULL,
    .readdir = NULL,
    .mkdir = NULL,
    .create = NULL,
...
int fat_mount(void) {
    ...
    first_root_dir_sector = first_fat_sector + (uint32_t)bpb->num_fats * fat_size_16;
